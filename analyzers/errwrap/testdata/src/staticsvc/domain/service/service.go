...
func (s *Service) badVar() error {
	return model.ErrSnapshotNotFound // want `GID-177: статичная ошибка возвращается без стека — оберните errors\.WithStack \(или errors\.Wrap, если нужен контекст\)`
}

// --- Позитив: адрес именованного error-типа без обёртки ---

func (s *Service) badPtrLit() error {
	return &model.BigError{Code: 1} // want `GID-177: статичная ошибка возвращается без стека — оберните errors\.WithStack \(или errors\.Wrap, если нужен контекст\)`
}

// --- Позитив: композит-литерал именованного error-типа без обёртки ---

func (s *Service) badValueLit() error {
	return model.BigError{Code: 2} // want `GID-177: статичная ошибка возвращается без стека — оберните errors\.WithStack \(или errors\.Wrap, если нужен контекст\)`
}
