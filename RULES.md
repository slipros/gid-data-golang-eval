...
| GID-242 | no-error-map-func | A dedicated error-**mapper** function is forbidden: a top-level `FuncDecl` that BOTH classifies its own NAMED `error` parameter via `errors.Is(<param>, ...)` **or** `errors.As(<param>, ...)` (stdlib `errors`) AND **returns `error`** (`func mapX(err error) error`, `func mapX(err error) (T, error)`) — it maps errors from layer to layer. No exceptions — map the bounded error set inline, at the call site (handler/interceptor) where the error occurs. Discriminators: a **bool-predicate** (`func isNotFound(err error) bool`, `func isRetryable(err error) bool`) classifies but does not return `error` → legitimate, not flagged; inline handling that switches on a **local** variable (not the function's own parameter) is not affected (linter `giderrmapfunc`) | owner decision 2026-07-12 | ✅ | ✅ |
