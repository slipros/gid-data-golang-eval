...
| GID-242 | no-error-map-func | A dedicated function that classifies its own error parameter via `errors.Is` is forbidden — **not specific to any target** (gRPC status, HTTP status, a log level, a business code, another error, ...): a top-level `FuncDecl` with a NAMED `error` parameter whose body calls `errors.Is(<that parameter>, ...)` anywhere. No exceptions — handle the bounded set of errors inline, at the call site (handler/interceptor) where the error occurs, whatever the target. Inline handling that switches on a **local** variable (not the function's own parameter) is not affected (linter `giderrmapfunc`) | owner decision 2026-07-12 | ✅ | ✅ |
| GID-243 | error-return-zero-value | On error, every non-error result of a `return` with ≥2 results must be nil/zero: applies when the last result is a **definitely non-nil** error — a constructing call (`status.Error`/`Errorf`, `errors.New`/`Wrap`/`Wrapf`/`Errorf`/`WithStack`/`WithMessage`/`WithMessagef`, `fmt.Errorf`) or a `return` lexically inside `if <e> != nil { ... }` returning that same `<e>`. "Zero" — `nil`, `false`, a zero basic literal (`0`, `0.0`, `""`), an empty composite literal `T{}`; a variable, a populated literal, or `&T{}` (a non-nil pointer) are not zero. An unconditional final forward (`return resp, err`, not inside an `if err != nil` guard) is a legitimate pass-through and is not flagged. No exceptions (linter `giderrzeroret`) | owner decision 2026-07-12 | ✅ | ✅ |
